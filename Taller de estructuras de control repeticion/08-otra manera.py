...
def Encuesta_licor():
    ...
    while True:
        while True:
            pregunta_dicotomica = input("Consume alcohol? (Si/No): ").capitalize()
        ...
        if pregunta_dicotomica == 'No':
            total_encuestados += 1
            continuar = input("¿Desea continuar con otra encuesta? (Sí/No): ").lower()
        ...
        while True:
            licor_preferido = input(f"Licor preferido (Opciones: {', '.join(licor)}): ").capitalize()
    ...
    promedio_edad_cerveza = sum(edad_cerveza) / len(edad_cerveza) if edad_cerveza else 0
    porcentaje_whisky = (consumen_whisky / total_encuestados * 100) if total_encuestados > 0 else 0
    ...
    print(f"Total de hombres que no consumen aguardiente y tienen entre 20 y 25 años: {consumo_20_25_años}")
